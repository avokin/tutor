class UserCategory < ActiveRecord::Base
  ...
  def self.all_with_info(user)
    UserCategory.joins(:user_words).select('user_category_id as id', :is_default, :name, 'count(user_category_id) as words_count', "sum(case when time_to_check < '#{DateTime.now }' then 1 else 0 end) as ready_words_count").group(:user_category_id).where(:user_id => user.id, :language_id => user.target_language.id)
  end
